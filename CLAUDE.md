...
- Side-slot virtual columns: `flattenSideSlots()` flattens the first element of side-detail compound arrays into positional `side1.*` (buy/client) and `side2.*` (sell/firm) columns so trades and fulfillments line up in the same columns; emission is deterministic per event type (driven by CAT_ARRAY_ELEMENTS) so `discoverColumns()`'s one-record-per-type sampling always finds the keys. All slot subfields are visible by default, grouped per slot in PRIORITY_COLUMNS after tradeID; the orderID cells carry a role chip (B/S/C/F for the source array) and a "+N" chip when the array has extra elements. The raw compound columns are in DEFAULT_HIDDEN_COLUMNS — hidden unless the saved column order already knows the column (so saved layouts keep user choices). Slot columns behave as regular columns (sort/filter/search via combobox, CSV export, orderID linkage) and surface their source field's validation errors at the cell level; they are excluded from `_searchText` (values duplicate the compound field), the detail panel, and the Raw Data tab. Slot metadata lives in `record._sideSlots`
- Clickable linkage fields (orderID, tradeID, fulfillmentID/priorFulfillmentID, allocationID/priorAllocationID, quoteID/routedQuoteID/receivedQuoteID, RFQID, parentOrderID, priorOrderID, routedOrderID, etc.) for navigating between related records; clicking clears other active filters. Events without a top-level orderID (Order Trade / Order Fulfillment: MEOT, MEOTS, MOOT, MEOF, MEOFS, MEFA, MOOF, MOOFS, MOFA) are linked to orders via the orderIDs inside their side details (buyDetails/sellDetails/clientDetails/firmDetails). Nested orderID linkage is type-agnostic: `getNestedOrderIDs()` extracts orderIDs from any compound array field on any event type (side details plus aggregatedOrders/askAggregatedOrders/bidAggregatedOrders, in both JSON array and CSV string form), and `getRecordLaneOrderIDs()` resolves a record's effective orderIDs from it — used by the ID index, order chain views, and timeline lanes. The orderID filter matches nested orderIDs too, so filtering by an order ID includes its trade/fulfillment events (e.g. MENO/MECO/MEOR and the MEOT trades referencing the same order), and the orderID filter dropdown lists orderIDs that appear only inside compound arrays. Free-text search covers nested compound field values.
- Order chain view with hierarchy tree, depth controls (This order, + Children, + Branch) that toggle off when clicked again; + Children shows all descendants; Branch shows direct ancestor chain plus descendants, excluding siblings. When events in an order group share a timestamp, order-origination events (MENO/MEOA/MECO/MEIR and options/multi-leg equivalents) sort first, then their supplements (MENOS/MONOS/MLOS), then other events
...
### Sample data
- `1234_TEST_20250317_Sample_OrderEvents_000001.json` — 84 records covering MEAA, MECO, MEFA, MEIM, MEIR, MENO, MENOS, MEOA, MEOC, MEOF, MEOFS, MEOM, MEOR, MEOT, MLNO, MLOR, MOCO, MONO, MOOF, MOOT events. Includes a fulfillment scenario (client order TE-001 filled from representative order TE-012 via MEOF/MEOFS, amended by MEFA), a trade with buy-side details (TRD-TE-004 → TE-007), and an options fulfillment (OPT-001 filled from firm order OPT-009 via MOOF) — these exercise side-detail orderID linkage in timelines, order chains, and clickable links. Also includes three same-timestamp groups written in deliberately scrambled file order to exercise the order chain tie-breaker (origination event first, then supplements): TE-014 (MEOT/MEOR/MENOS before MENO, with MENOS aggregating client order TE-015), TE-CH-003 (MEOC before MECO), and TE-IR-004 (MEOR before MEIR)

### Testing
- Run with `node --test tests/` (Node 18+, no dependencies)
- `tests/chain-order.test.js` extracts code under test (CHAIN_ORIGIN_TYPES, compareChainRecords, CAT_ALLOWED_TYPES) from `index.html` by regex and evaluates it in isolation — renaming those identifiers or restructuring their declarations breaks extraction, and the tests fail loudly when that happens. It covers chain event ordering (origination events, then supplements, then others within a timestamp tie), sample file integrity (valid JSON/types, unique firmROEIDs, timestamp format), and the scrambled same-timestamp fixture groups (TE-014, TE-CH-003, TE-IR-004) — those fixtures must keep the origination event last in file order or the tests reject them as proving nothing
